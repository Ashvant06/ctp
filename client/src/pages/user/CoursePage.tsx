...
export default function CoursePage() {
  ...
  useEffect(() => {
    supabase.from("courses").select("*, sections(*, lessons(*))").eq("id", id!).single()
      .then(({ data }) => {
        if (data) {
          data.sections = data.sections
            .sort((a: Section, b: Section) => a.order_index - b.order_index)
            .map((s: Section) => ({ ...s, lessons: s.lessons.sort((a: Lesson, b: Lesson) => a.order_index - b.order_index) }));
          setCourse(data);
          const first = data.sections[0]?.lessons?.find((l: Lesson) => l.status === "ready");
  ...
  useEffect(() => {
    if (!id) return;
    const channel = supabase.channel("lesson-status")
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "lessons", filter: `course_id=eq.${id}` },
        payload => {
          setCourse(prev => !prev ? prev : {
            ...prev,
            sections: prev.sections.map(s => ({
              ...s,
              lessons: s.lessons.map(l => l.id === payload.new.id ? { ...l, ...payload.new } : l)
            }))
          });
        })
      .subscribe();
          ...
          {activeLesson ? (
            activeLesson.status === "ready" ? (
              ...
              <div style={s.processingBox}>
                ...
                <p style={s.processingText}>Video is {activeLesson.status}...</p>
                <p style={s.processingSubText}>This may take a few minutes. The page will update automatically.</p>
              </div>
            )
          ) : (
            <div style={s.processingBox}>
            ...
            <div style={s.infoBar}>
              <div>
                ...
                <p style={s.lessonMeta}>
                  {course.sections.find(s => s.lessons.some(l => l.id === activeLesson.id))?.title}
                </p>
              </div>
              <div style={s.navBtns}>
                <button
                  onClick={() => prevLesson?.status === "ready" && setActiveLesson(prevLesson)}
                  disabled={!prevLesson || prevLesson.status !== "ready"}
                  style={{ ...s.navBtn, opacity: !prevLesson ? 0.3 : 1 }}
                >
                  ← Prev
                </button>
                <button
                  onClick={() => nextLesson?.status === "ready" && setActiveLesson(nextLesson)}
                  disabled={!nextLesson || nextLesson.status !== "ready"}
                  style={{ ...s.navBtn, opacity: !nextLesson ? 0.3 : 1 }}
                >
                  Next →
                </button>
              </div>
                ...
                <div key={section.id}>
                  ...
                  {section.lessons.map((lesson, li) => {
                    ...
                    return (
                      <div
                        key={lesson.id}
                        onClick={() => isReady && setActiveLesson(lesson)}
                        style={{
                          ...s.lessonItem,
                      ...
                      >
                        <span style={{ ...s.lessonItemIcon, color: isActive ? "var(--accent)" : "var(--text-muted)" }}>
                          {isReady ? "▶" : "⏳"}
                        </span>
                        <div style={s.lessonItemContent}>
                          <span style={{ ...s.lessonItemTitle, color: isActive ? "var(--text-primary)" : "var(--text-secondary)" }}>
                            {li + 1}. {lesson.title}
                          </span>
                          {!isReady && (
