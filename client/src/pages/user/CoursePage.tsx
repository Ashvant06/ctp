...
export default function CoursePage() {
  ...
  const fetchPlayUrl = async (lesson: Lesson): Promise<string> => {
    // Use Supabase JS SDK directly — bypasses Go signed URL issues
        ...
        <div style={s.playerArea}>
            ...
            activeLesson.status === "ready" ? (
              urlLoading ? (
                ...
                <div style={s.processingBox}>
                  ...
                  <button onClick={() => handleSelectLesson(activeLesson)} style={s.retryBtn}>
                    Retry
                  </button>
                </div>
              )
          ...
          {activeLesson && (
            <div style={s.infoBar}>
              ...
              <div style={s.navBtns}>
                <button
                  onClick={() => prevLesson?.status === "ready" && handleSelectLesson(prevLesson)}
                  disabled={!prevLesson || prevLesson.status !== "ready"}
                  style={{ ...s.navBtn, opacity: !prevLesson || prevLesson.status !== "ready" ? 0.3 : 1 }}
                >
                  ← Prev
                </button>
                <button
                  onClick={() => nextLesson?.status === "ready" && handleSelectLesson(nextLesson)}
                  disabled={!nextLesson || nextLesson.status !== "ready"}
                  style={{ ...s.navBtn, opacity: !nextLesson || nextLesson.status !== "ready" ? 0.3 : 1 }}
                >
                  Next →
                </button>
              </div>
